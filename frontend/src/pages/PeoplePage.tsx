...
import PeopleProvider, { usePeopleContext } from "@/providers/PeopleProvider";
...
function PeoplePageContent() {
  ...
  useEffect(() => {
    const filtered = people.filter((person) =>
      normalize(person.nome).includes(normalize(searchTerm))
    );
  ...
  const totalWithContacts = people.filter(
    (person) => !!person?.contatos?.length
  ).length;
